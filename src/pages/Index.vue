    ...
    <q-card style="padding: 1em; font-family: 'Comic Sans MS'">
      <q-list>
        <q-collapsible group="sample" icon="search" label="Preparation">
          <div>
            <div>
              Create two new files under pages/game directory by copying sample.vue and sampleContainer.vue
              <li>gameName.vue</li>
              <li>gameNameContainer.vue</li>
            </div>
            <div>
              Create a new directory statics/game/gameName
            </div>
            <div>
              Modify sample-game, and SampleGame to game-name and GameName in gameNameContainer.vue
            </div>
            <div>
              Copy image and image json file to statics/game/gameName
            </div>
            <div>
              Add a new route in src/router/routes.js
        ...
        <q-collapsible group="sample" icon="search" label="Set Background">
          <div>
            Add "background.png" to the assets array
          </div>
          <div>
            Add the following lines in initGame function
...
this.contentLayer.addChild(_button)</pre>
        ...
        <q-collapsible group="sample" icon="search" label="Set question and standard option">
          <div>
            Add a function createButton and call this function in function initGame
            <div class="code-container">
              <code>
...
optionClicked (index) {
  ...
  this.waitForNextQuestion()
          ...
          <div style="padding: 1em;">
            <q-btn color="primary" no-caps @click="openGame({setQuestionAndOption: true, checkResult: true})">Show</q-btn>
          </div>
        </q-collapsible>
        <q-card-separator />
...
createOptions () {
  for (let i = 0; i &lt; 3; i++) {
    ...
    this.contentLayer.addChild(this.containers[i])
          ...
          <div style="padding: 1em;">
            <q-btn color="primary" no-caps @click="openGame({setOption: true, checkResult: true})">Show</q-btn>
          </div>
        </q-collapsible>
        <q-card-separator />
        <q-collapsible group="sample" icon="search" label="Show arrow control">
          <div>
            Call this.setArrowControl (4 arrows) or this.setLeftRightControl (left, right arrows) in initGame
          </div>
          <div>
            Set these function to receive arrow control event
            <div class="code-container">
              <code>
...
moveDownStop () { },</pre>
          ...
          <div style="padding: 1em;">
            <q-btn color="primary" no-caps @click="openGame({setArrowControl: true})">Show (4 arrows)</q-btn>
            <q-btn color="primary" no-caps @click="openGame({setLeftRightControl: true})">Show (left, right arrows)</q-btn>
          </div>
        </q-collapsible>
        <q-card-separator />
        <q-collapsible group="sample" icon="search" label="Control animation">
          <div>
            <div>
              To control the speed and looping, set animations in gameName.json
            </div>
    ...
    "blueWalk": { "frames": [8, 10, 12, 13], "next": "blueWalk", "speed": 0.1 }
        ...
        <q-collapsible group="sample" icon="search" label="Move along path">
          <div>
            ...
            <div style="padding: 1em;">
              <q-btn color="primary" no-caps @click="openGame({setBackground: true, showPathCreation: true})">Show</q-btn>
            </div>
            <div>
              Play animation path
            </div>
            <div style="padding: 1em;">
              <q-btn color="primary" no-caps @click="openGame({setBackground: true, playPathAnimation: true})">Show</q-btn>
            </div>
          </div>
        </q-collapsible>
        <q-card-separator />
        ...
        <q-collapsible group="sample" icon="search" label="Sample Games">
          <div>
            <div>
              ...
              <q-btn color="primary" no-caps @click="openSampleGame('ninjaGhostLeg')">Ninja Ghost Leg</q-btn>
    ...
    <q-modal v-model="sampleGameOpened" maximized>
      <div style="position: relative;">
        ...
        <div style="position: absolute; top: 0px; right: 0px;"><q-btn
          color="primary"
          @click="sampleGameOpened = false; currentGame = ''"
          label="Close"
        /></div>
      </div>
    </q-modal>
